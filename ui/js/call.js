...
export function setCallUI(on) {
    ...
    const b = $("#btnCall");
    if (b)
        b.textContent = on ? "📵 Raccrocher" : "📞 Appeler";
    const s = $("#callStatus");
    if (s)
        s.textContent = on ? "🔊 En appel — parle ! (casque conseillé pour éviter l'écho)" : "";
    const m = $("#btnMute");
    if (m) {
...
export function initCall() {
    ...
    listen("ghost-audio-error", (e) => {
        if (!e.payload || e.payload.scope !== "call" || !S.inCall)
            return;
