...
function flushPInv(member) {
    ...
    mine.forEach((x) => invoke("send_ginvite", { member: x.member, gid: x.gid, name: x.name, members: x.csv })
...
function watchingGroup(id) {
    if (!id || S.openGroupId !== id || !S.focused)
        return false;
...
export function clearUnreadIfWatching(id) {
    if (!id || !watchingGroup(id) || !S.unread[id])
        return false;
    delete S.unread[id];
    return true;
}
...
function myVotes(gid) {
    try {
        const m = JSON.parse(localStorage.getItem("ghostlink_myvotes") || "{}");
        return m[gid] || [];
    }
    catch {
        return [];
...
function saveMyVote(gid, target, on) {
    let m = {};
    try {
        m = JSON.parse(localStorage.getItem("ghostlink_myvotes") || "{}");
    }
    catch {
        /* repart d'un objet vide */
    }
...
// ----- Chat de groupe -----
function addGroupMsgDom(author, text, who, from) {
    ...
    if (who !== "me") {
        ...
        if (label) {
            ...
            if (from)
                au.dataset.from = from; // cible du ré-étiquetage en place (relabelPeer)
...
function getPc(peer) {
    ...
    pc.onconnectionstatechange = () => {
        ...
        if (["failed", "closed"].includes(pc.connectionState)) {
            ...
            if (S.pcs[peer] && S.pcs[peer].pc === pc)
                delete S.pcs[peer];
        }
    };
    return st;
...
async function startCam() {
    if (camBusy)
        return; // capture déjà en cours de démarrage (double-clic)
...
async function openNativePicker(g) {
    ...
    const rSel = $("#nativePickerRes");
    rSel.replaceChildren();
    RES_PRESETS.forEach((r) => {
        const o = document.createElement("option");
        o.value = r.key;
        o.textContent = r.label;
        if (r.key === currentRes().key)
            o.selected = true;
        rSel.appendChild(o);
    });
...
export function initGroups() {
    ...
    listen("ghost-audio-error", (e) => {
        if (!e.payload || e.payload.scope !== "group" || !S.inGroupCall)
            return;
        stopGroupCall();
    ...
    setInterval(() => {
        ...
        if (!p.length)
            return;
        p.forEach((x) => invoke("send_ginvite", { member: x.member, gid: x.gid, name: x.name, members: x.csv })
            .then(() => savePInv(loadPInv().filter((y) => !(y.member === x.member && y.gid === x.gid))))
            .catch(() => { }));
    }, 60000);
    ...
    document.addEventListener("gl-relabel", (e) => relabelPeerTiles(e.detail));
    // Listeners groupes
            ...
            if (S.inGroupCall && S.groupCallId) {
                ...
                if (gc && gc.members.includes(peer)) {
                    invoke("group_call_sync", { members: gc.members })
                        ...
                        if (v != null && v !== 100)
                            invoke("group_call_volume", { peer, vol: v / 100 }).catch(() => { });
                        applyStreamGain(peer);
    ...
    listen("ghost-gchat-img", (e) => {
        ...
        addImgBubble($("#groupChatLog"), `data:${p.mime};base64,${p.dataB64}`, "them", p.from ? memberName(p.from, p.author) : clampLabel(p.author), p.from, !!(p.from && isDeclaredLabel(p.from, p.author)));
    });
