...
export function initSession(): void {
  ...
  $("#btnAccept").onclick = () => {
    ...
    if (S.currentPeer && !confirm("Tu es déjà connecté à un pair.\nAccepter cette nouvelle connexion FERMERA la session actuelle.\n\nContinuer ?")) {
      if (S.incomingId != null) invoke("respond_incoming", { id: S.incomingId, accept: false }).catch(() => {});
      $("#incomingBanner").classList.add("hidden");
      S.incomingId = null;
      log("Nouvelle connexion refusée — session actuelle conservée.");
      return;
    }
    if (S.incomingId != null) invoke("respond_incoming", { id: S.incomingId, accept: true }).catch(() => {});
    $("#incomingBanner").classList.add("hidden");
    S.incomingId = null;
  ...
  listen("ghost-incoming-cancel", (e) => {
    ...
    if (id != null && S.incomingId != null && id !== S.incomingId) return;
    $("#incomingBanner").classList.add("hidden");
    S.incomingId = null;
  });
