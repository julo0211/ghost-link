...
const kickDenom: Record<string, number> = {};
function kickQuorum(g: Group, target?: string): number {
  const live = onlineMemberCount(g);
  // 60 % des en-ligne, plancher à 2 (jamais d'exclusion « solo »).
  if (!target) return Math.max(2, Math.ceil(0.6 * live));
...
function videoPrivacyOk(): boolean {
  ...
  const ok = confirm(
    "⚠️ Confidentialité — la vidéo WebRTC (caméra, et partage d'écran si le partage natif est désactivé) ouvre une connexion DIRECTE : ton adresse IP devient visible par les membres du groupe, et un serveur STUN public (Google) est contacté. Cela vaut aussi bien quand tu ÉMETS que quand tu REÇOIS le flux d'un membre. Le chat, les fichiers et la voix restent relayés. Activer la vidéo WebRTC quand même ?",
  );
...
function codesDeRoster(csv?: string): string[] {
  const vus = new Set<string>();
  for (const c of (csv || "").split(",")) {
    const code = c.trim().toLowerCase();
    if (CODE_PAIR.test(code)) vus.add(code);
    if (vus.size >= MAX_MEMBRES_GROUPE + 1) break; // +1 : le roster inclut son expéditeur
  }
  return [...vus];
}
...
function answerGfileOffer(accept: boolean): void {
  const o = S.gfileOffers.shift();
  if (o) invoke("respond_gfile", { id: o.id, accept }).catch((e) => log("Réponse à l'offre de fichier : " + e));
  paintGfileOffer();
}
...
export function initGroups(): void {
  ...
  listen("ghost-gchat-img", (e) => {
    ...
    const p = e.payload || ({} as { group?: string; author?: string; mime?: string; dataB64?: string; from?: string });
    const g = loadGroups().find((x) => x.id === p.group);
    if (!p.group || !g) return;
  ...
  listen("ghost-gcall", (e) => {
    const p = e.payload || ({} as { group?: string; from?: string });
    ...
    if (!p.from || !g.members.includes(p.from)) return;
    S.pendingGCall = g.id;
  ...
  listen("ghost-signal", async (e) => {
    ...
    if (!S.pcs[peer] && !videoPrivacyOk()) return;
    const st = getPc(peer);
    const pc = st.pc;
    try {
      if (msg.description) {
  ...
  listen("ghost-grecv-done", (e) => {
    ...
    const g = loadGroups().find((x) => x.id === S.openGroupId);
    const from = p.from || "";
    if (!mime || !p.path || !g || !g.members.includes(from)) return;
    invoke("read_image_bytes", { path: p.path })
      .then((buf) => {
        if (S.openGroupId !== g.id) return; // groupe changé pendant la lecture
  ...
  listen("ghost-gsend-result", (e) => {
    const p = e.payload;
    if (!p) return;
    if (p.ok) log("📤 « " + p.name + " » envoyé à " + memberName(p.peer) + ".");
    else log("⛔ « " + p.name + " » non reçu par " + memberName(p.peer) + " : " + (p.error || "échec") + ".");
  });
