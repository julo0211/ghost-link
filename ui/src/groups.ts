...
function showCanvasTile(key: string, label: string, peer?: string): HTMLCanvasElement {
  ...
  if (!w) {
    ...
    if (peer) {
      ...
      w.appendChild(snd);
      if (muted0) invoke("screen_audio_mute", { peer, on: true }).catch(() => {});
    }
    vgrid().appendChild(w);
  }
  vgrid().classList.remove("hidden");
...
function handleNativeSignal(peer: string, nv: { start?: boolean; w?: number; h?: number; fps?: number }): void {
  if (nv.start) {
...
async function startScreenNative(): Promise<void> {
  const g = loadGroups().find((x) => x.id === S.openGroupId);
  if (!g) return;
  screenBusy = true;
  try {
    const epoch0 = nativeShareEpoch;
    let info: { w: number; h: number; fps: number };
    try {
