...
export interface GroupMsg {
   ...
   *  Stocké pour résoudre le libellé AU RENDU quand le tampon est rejoué (changement de
   *  groupe), au lieu de rejouer une chaîne figée — c'est ce qui rend un renommage
   *  rétroactif. `who` reste `string` : le rétrécir casserait la chaîne d'appels. */
  from?: string;
  text: string;
  who: string;
}
