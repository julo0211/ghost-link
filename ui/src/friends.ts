...
export function renderFriends(): void {
  ...
  a.forEach((f, i) => {
    ...
    const nm = d.querySelector(".nm") as HTMLElement;
    ...
    (d.querySelector(".pth") as HTMLElement).textContent = S.fpCache[f.code] || shortId(f.code);
    (d.querySelector("[data-c]") as HTMLElement).onclick = () => connectTo(f.code);
    (d.querySelector("[data-x]") as HTMLElement).onclick = () => removeFriend(i);
    box.appendChild(d);
  });
}
