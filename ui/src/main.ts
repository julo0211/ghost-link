...
async function refreshMonitors(): Promise<void> {
  const sel = $<HTMLSelectElement>("#setNativeMonitor");
  try {
...
function initSettings(): void {
  ...
  $<HTMLSelectElement>("#setNativeMonitor").onchange = () => {
    const v = $<HTMLSelectElement>("#setNativeMonitor").value;
    localStorage.setItem("ghostlink_native_monitor", v);
    log("Écran du partage natif : " + ($<HTMLSelectElement>("#setNativeMonitor").selectedOptions[0]?.textContent || "principal") + " (au prochain partage).");
  };
  ...
  const skinSel = $<HTMLSelectElement>("#setSkin");
