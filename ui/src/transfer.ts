...
// `Uint8Array<ArrayBuffer>` (et non `Uint8Array` nu, qui admet un SharedArrayBuffer) : c'est
// ce que `Blob` exige, et c'est bien ce que rend un ArrayBuffer venu de l'IPC.
type LectureImage = { ok: true; bytes: Uint8Array<ArrayBuffer> } | { ok: false; tropGrande: boolean; raison: string };
...
async function lireImageDeposee(p: string): Promise<LectureImage> {
  try {
...
function accepteRepliFichier(nom: string, groupe: boolean): boolean {
  const ok = confirm(
    "« " +
      nom +
...
function imageIllisible(nom: string, raison: string, groupe: boolean): void {
  log(
    "🖼️ « " + nom + " » ne peut pas s'afficher dans la conversation (" + raison + ") — " +
...
async function deposer1a1(p: string, mime: string | null): Promise<void> {
  if (mime) {
    ...
    if (!accepteRepliFichier(baseName(p), false)) return;
    setFile(p);
...
async function deposerGroupe(p: string, mime: string | null): Promise<void> {
  ...
  if (mime) {
    ...
    if (!accepteRepliFichier(baseName(p), true)) return;
    $<HTMLInputElement>("#groupFilePath").value = p;
...
function answerFileOffer(accept: boolean): void {
  const o = S.fileOffers.shift();
  if (o) invoke("respond_file", { id: o.id, accept }).catch((e) => log("Réponse à l'offre de fichier : " + e));
  paintFileOffer();
}
...
function noteIncoming1to1(): void {
  playPing("msg");
...
export function initTransfer(): void {
  ...
  listen("ghost-recv-failed", (e) => {
    ...
    $("#recvBox").classList.add("hidden");
    log(
      "⚠️ « " + (p.name || "fichier") + " » accepté mais impossible à enregistrer (" + (p.error || "?") +
  ...
  listen("ghost-chat-img", (e) => {
    ...
    addImgBubble(
      ...
      peer || undefined,
      !!(peer && isDeclaredLabel(peer, p.author)),
    );
  });
